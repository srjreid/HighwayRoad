...
#define FT_FILENAME( x )  ( (x) ? (x) : "unknown file" )
  ...
  ft_mem_table_new( FT_Memory  memory )
  {
    ...
    table->alloc   = memory->ft_alloc;
    table->realloc = memory->ft_realloc;
    table->free    = memory->ft_free;
  ...
  ft_mem_debug_init( FT_Memory  memory )
  {
    ...
    if ( getenv( "FT2_DEBUG_MEMORY" ) )
    {
      ...
      if ( table )
      {
        ...
        memory->user    = table;
        memory->ft_alloc   = ft_mem_debug_alloc;
        memory->ft_realloc = ft_mem_debug_realloc;
        memory->ft_free    = ft_mem_debug_free;
  ...
  ft_mem_debug_done( FT_Memory  memory )
  {
    ...
    if ( table )
    {
      memory->ft_free    = table->free;
      memory->ft_realloc = table->realloc;
      memory->ft_alloc   = table->alloc;
