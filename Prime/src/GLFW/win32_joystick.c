...
static BOOL CALLBACK deviceCallback(const DIDEVICEINSTANCE* di, void* user)
{
    ...
    if (memcmp(&di->guidProduct.Data4[2], "PIDVID", 6) == 0)
    {
        ...
        if(di->guidProduct.Data1 == 0x05C4054C
          || di->guidProduct.Data1 == 0x09CC054C
          || di->guidProduct.Data1 == 0x0BA0054C) {
          WideCharToMultiByte(CP_UTF8, 0, L"Sony DualShock4", -1, name, sizeof(name), NULL, NULL);
        }
        else if(di->guidProduct.Data1 == 0x0CE6054C) {
          WideCharToMultiByte(CP_UTF8, 0, L"Sony DualSense", -1, name, sizeof(name), NULL, NULL);
        }
        else if((di->guidProduct.Data1 & 0xFFFF) == 0x054C) {
          WideCharToMultiByte(CP_UTF8, 0, L"Sony Detected", -1, name, sizeof(name), NULL, NULL);
        }
        else if(di->guidProduct.Data1 == 0x2006057E) {
          WideCharToMultiByte(CP_UTF8, 0, L"Nintendo Joy-Con L", -1, name, sizeof(name), NULL, NULL);
        }
        else if(di->guidProduct.Data1 == 0x2007057E) {
          WideCharToMultiByte(CP_UTF8, 0, L"Nintendo Joy-Con R", -1, name, sizeof(name), NULL, NULL);
        }
        else if(di->guidProduct.Data1 == 0x2009057E) {
          WideCharToMultiByte(CP_UTF8, 0, L"Nintendo Switch Pro Controller", -1, name, sizeof(name), NULL, NULL);
        }
        else if((di->guidProduct.Data1 & 0xFFFF) == 0x057E) {
          WideCharToMultiByte(CP_UTF8, 0, L"Nintendo Detected", -1, name, sizeof(name), NULL, NULL);
        }
    }
    else
    {
...
int _glfwPlatformPollJoystick(_GLFWjoystick* js, int mode)
{
    ...
    else
    {
        ...
        _glfwInputJoystickAxis(js, 4, xis.Gamepad.bLeftTrigger / 255.5f);
        _glfwInputJoystickAxis(js, 5, xis.Gamepad.bRightTrigger / 255.5f);
